...
{
    public class StateMachineBrawl : MonoBehaviour
    {
        ...
        private readonly int weaponDistance = 2;
        ...
        protected virtual void Update()
        {
            ...
            //select target
            Vector3 target = GetFocusPlayer();
            float dist = Vector3.Distance(playerController.transform.localPosition, target);
            ...
            {
                //jump
                if (target.y > playerController.transform.localPosition.y && (target.y - playerController.transform.localPosition.y) > weaponDistance)
                {
                    jump = true;
                }

                //move
                if ((target.x - playerController.transform.localPosition.x) <= weaponDistance)
                {
                    move = false;
                }

                //direction
                if ((target.x - playerController.transform.localPosition.x) < 0)
                {
                    right = true;
                }
                else
        ...
        private bool CreateNewState(Enum newstate)
        {
            ...
            s.enterMethod = StateMachineBrawl.GetMethodInfo<Action<Enum>>(this, type, "Enter" + newstate, DoNothingEnterExit);
            s.updateMethod = StateMachineBrawl.GetMethodInfo<Action>(this, type, "Update" + newstate, DoNothingUpdate);
            s.exitMethod = StateMachineBrawl.GetMethodInfo<Action<Enum>>(this, type, "Exit" + newstate, DoNothingEnterExit);
        ...
        private Vector3 GetFocusPlayer()
        {
            int nbPlayer = BrawlCore.Instance.players.Length;
            Vector3 myVector = playerController.transform.localPosition;
            Vector3 result = new Vector3();
            float dist = 0;
            for (int i = 0; i < nbPlayer; i++)
            {
                if(playerController.playerId != BrawlCore.Instance.players[i].playerId)
                {
                    Vector3 v = BrawlCore.Instance.players[i].transform.localPosition;
                    var distTemp = Vector3.Distance(v, myVector);
                    if (i == 0 || Math.Abs(dist) > Math.Abs(distTemp))
                    {
                        dist = distTemp;
                        result = v;
                    }
